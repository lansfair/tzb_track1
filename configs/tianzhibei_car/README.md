...
```text
lsknet-s_fpn_kfiou_staged_40e.py
```

- RoI 旋转框回归使用 KFIoU（`KFLoss(fun='ln', loss_weight=5.0)`）。
- Oriented RPN 水平候选框回归仍使用 Smooth L1。
...
```text
mtp-vit-l-rvsa_kfiou_staged_40e.py
```

使用转换后的 FAIR1M 检测权重，并沿用相同 A/B/C 数据策略。MTP backbone
从相邻的 `tianzhibei-inference` 仓库自动加载。RoI 旋转框回归同样使用
KFIoU，Oriented RPN 回归仍使用 Smooth L1。
